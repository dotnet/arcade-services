import { browser, logging, element, by, ExpectedConditions as until } from "protractor";
...
describe("App", () => {
  ...
  it("is accessible", async () => {
    const results = await new AxeWebdriverjs(driver)
      .include("body")
      .withTags(['wcag2a', 'wcag2aa', 'wcag21aa'])
      .analyze();
    await exportAxeAsSarifTestResult('index.sarif', results);
  ...
  async function exportAxeAsSarifTestResult(sarifFileName: string, axeResults: Axe.AxeResults): Promise<void> {
    ...
    await fs.mkdir(testResultsDirectory, {
      recursive: true
    });

    const sarifResultFile = path.join(testResultsDirectory, sarifFileName);
    await fs.writeFile(sarifResultFile, JSON.stringify(sarifResults, null, 2));
