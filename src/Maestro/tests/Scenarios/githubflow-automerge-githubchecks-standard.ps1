...
try {
    Write-Host
    Write-Host "GitHub Dependency Flow, non-batched, standard merge policy"
    Write-Host
    ...
    $subscriptionId = Darc-Add-Subscription-And-Trigger --channel "$testChannelName" --source-repo "$sourceRepoUri" --target-repo "$targetRepoUri" --update-frequency none --target-branch "$targetBranch" --standard-automerge
