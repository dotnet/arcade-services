...
try {
    ...
    $subscriptionId = Darc-Add-Subscription -darcParams @( "--channel", "$testChannelName", "--source-repo", "$parentSourceRepoUri", "--target-repo", "$targetRepoUri", "--update-frequency", "none", "--target-branch", "$targetBranch" )
    ...
    try {
        ...
        Darc-Command -darcParams @( "add-dependency", "--name", "Foo", "--type", "product", "--repo", "$parentSourceRepoUri" )
        Darc-Command -darcParams @( "add-dependency", "--name", "Bar", "--type", "product", "--repo", "$parentSourceRepoUri" )
        Darc-Command -darcParams @( "add-dependency", "--name", "Baz", "--type", "product", "--repo", "$childSourceRepoUri", "--coherent-parent", "Foo" )
    }
    finally {
        Pop-Location
    }
