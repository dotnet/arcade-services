...
try {
    Write-Host
    Write-Host "Azure DevOps Dependency Flow, batched"
    Write-Host
    ...
    $success = Check-Batched-AzDO-PullRequest $source1RepoName $source2RepoName $targetRepoName $targetBranch $expectedDependencies
    if (!$success) {
