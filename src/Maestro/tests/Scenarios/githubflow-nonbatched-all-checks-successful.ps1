...
try {
    ...
    $subscriptionId = Darc-Add-Subscription -darcParams @("--channel", "$testChannelName", "--source-repo", "$sourceRepoUri", "--target-repo", "$targetRepoUri", "--update-frequency", "none", "--target-branch", "$targetBranch", "--all-checks-passed", "--ignore-checks", "license/cla" )
