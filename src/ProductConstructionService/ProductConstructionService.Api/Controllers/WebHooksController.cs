...
public class WebHooksController : Controller
{
    ...
    {
        foreach (Maestro.Data.Models.Repository repo in await Context.Repositories.Where(r => r.InstallationId == installationId).ToListAsync())
        {
            Context.Update(repo);
    ...
    {
        var token = await GitHubTokenProvider.GetTokenForInstallationAsync(installationId);
        ...
        {
            Maestro.Data.Models.Repository? existing = await Context.Repositories.FindAsync(repo.HtmlUrl);
            if (existing == null)
            {
