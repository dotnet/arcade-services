...
internal abstract class PullRequestActor : IPullRequestActor
{
        ...
        ILogger logger)
    {
        ...
        _pullRequestUpdateState = new(stateManager, reminders, _logger, id.Id + PullRequestUpdateKey);
        _pullRequestCheckState = new(reminders, id.Id + PullRequestCheckKey);
        _pullRequestState = new(stateManager, _logger, id.Id + PullRequestKey);
        _codeFlowState = new(stateManager, _logger, id.Id + CodeFlowKey);
    }
    ...
    public async Task<bool> ProcessPendingUpdatesAsync()
    {
        ...
        if (pr == null)
        {
            ...
            if (prUrl == null)
            {
                _logger.LogInformation($"No changes required for {subscriptionIds}, no pull request created");
            }
            else
            {
                _logger.LogInformation($"Pull request '{prUrl}' for {subscriptionIds} created");
            }
