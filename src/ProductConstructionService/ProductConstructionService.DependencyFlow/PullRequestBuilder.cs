...
internal class PullRequestBuilder : IPullRequestBuilder
{
        ...
        BuildDTO build)
    {
        ...
        {
            subscriptionSection.AppendLine($"  - RelativePath: {(UnixPath.IsEmptyPath(relativePath) ? "root" : relativePath)}");
        ...
        Dictionary<UnixPath, List<DependencyUpdate>> coherencyUpdatesPerDirectory)
    {
        ...
        {
            coherencySection.AppendLine($"  - in {(UnixPath.IsEmptyPath(targetDirectory) ? "root" : targetDirectory)}");
