...
public class GitHubClient : RemoteRepoBase, IRemoteGitRepo
{
    ...
    private async Task CacheResourceSafelyAsync<T>(string key, T value) where T: IRemoteGitResource
    {
        if (_entityCache == null)
        {
            return;
    ...
    .FirstOrDefault(h => h.Key.Equals("ETag", StringComparison.OrdinalIgnoreCase)).Value;
