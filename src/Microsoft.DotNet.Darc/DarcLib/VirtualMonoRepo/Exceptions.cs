...
public class BackflowNonContinuableNonLinearCodeflowException(string currentVmrSha, string lastFFRepoSha, string currentRepoSha)
    : DarcException($"Cannot unsafe backflow commit {currentVmrSha} because the current repo SHA {currentRepoSha} is not a descendant of the last forward-flown repo SHA {lastFFRepoSha}. "
