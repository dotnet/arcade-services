...
{
    Task MergeBack(string commitMessage);
...
public class WorkBranch : IWorkBranch
{
    ...
    public async Task MergeBack(string commitMessage)
    {
        _logger.LogInformation("Merging {branchName} into {mainBranch}", _workBranch, _originalBranch);

        await _localGitClient.CheckoutNative(_repoPath, _originalBranch);
        ...
        await _localGitClient.Commit(_repoPath, commitMessage, true, Constants.DotnetBotIdentity);
    }
}
