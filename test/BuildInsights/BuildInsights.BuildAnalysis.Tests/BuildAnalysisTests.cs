...
public class BuildAnalysisTests
{
    ...
    private class TestData : IDisposable, IAsyncDisposable
    {
        ...
        {
            private readonly ImmutableDictionary<Guid, ImmutableList<TimelineRecord>> _previousTimelines = [];
            private readonly ImmutableList<TimelineRecord> _timeline = [];
            private readonly ImmutableList<TestRunDetails> _testCaseResultFailuresBuild = [];
            private readonly ImmutableList<TestRunDetails> _testCaseResultsLatestRun = [];
            private readonly ImmutableList<TestRunDetails> _testPassedOnRerun = [];
            private readonly ImmutableDictionary<int, TestCaseResult> _testResultsHistory = [];
            private readonly ImmutableDictionary<string, ImmutableList<TestHistoryByBranch>> _testHistory = [];
            private readonly ImmutableList<Build> _latestBuilds = [];
            private readonly ImmutableList<Build> _builds = [];
            private readonly ImmutableList<KnownIssue> _knownIssues = [];
            ...
            public Builder WithTestCaseResultsLatestRun(List<TestRunDetails> testCaseResult) => With(testCaseResultsLatestRun: [.. testCaseResult]);

            public Builder WithTimeline(List<TimelineRecord> timeline) => With(timeline: [.. timeline]);

            public Builder WithTestCaseResultFailuresBuild(List<TestRunDetails> testCaseResults) => With(testCaseResultFailuresBuild: [.. testCaseResults]);

            public Builder WithTestPassedOnRerun(List<TestRunDetails> testPassedOnRerun) => With(testPassedOnRerun: [.. testPassedOnRerun]);
            ...
            public Builder WithTestHistory(string key, List<TestHistoryByBranch> testHistory) => With(testHistory: _testHistory.Add(key, [.. testHistory]));
            ...
            public Builder WithPreviousTimeline(string key, List<TimelineRecord> timeline) =>
                With(previousTimelines: _previousTimelines.Add(CreateGuid(key), [.. timeline]));

            public Builder WithLatestBuilds(List<Build> builds) => With(latestBuilds: [.. builds]);
            ...
            public TestData Build()
            {
                ...
                githubIssuesServiceMock.Setup(t => t.GetRepositoryKnownIssues(It.IsAny<string>())).ReturnsAsync([.. _knownIssues]);
    ...
    private class BuildBuilder
    {
        ...
        private readonly ImmutableList<BuildValidationResult> _validationResults = [];
    ...
    public async Task KnownIssueWithErrorMessageMatch()
    {
        ...
        TestData.Builder builder = TestData.Default
            .AddBuild(BuildBuilder.Default.WithResult(BuildResult.Failed).Build())
            .WithKnownIssues([KnownIssueHelper.ParseGithubIssue(MockIssue(issueBody), "testRepo", KnownIssueType.Infrastructure)])
            .WithTimeline(
            [
                MockRecord(
                    name: "A",
    ...
    public async Task BuildMessageWithEmptyBody()
    {
        TestData.Builder builder = TestData.Default
            .AddBuild(BuildBuilder.Default.WithResult(BuildResult.Failed).Build())
            .WithKnownIssues([KnownIssueHelper.ParseGithubIssue(MockIssue(""), "testRepo", KnownIssueType.Infrastructure)])
            .WithTimeline(
            [
                MockRecord(
                    name: "A",
    ...
    private static Octokit.Issue MockIssue(string body)
    {
