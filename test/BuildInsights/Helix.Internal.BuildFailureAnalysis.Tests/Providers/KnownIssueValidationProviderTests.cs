...
public partial class KnownIssueValidationProviderTests
{
    ...
    public async Task BodyMissingKnownIssueTest()
    {
        ...
        await testData.Processor.ValidateKnownIssue(TestKnownIssueValidationMessage(), CancellationToken.None);
...
```";
        var knownIssueValidationMessage = new KnownIssueValidationMessage
        {
            IssueId = 1234,
            Organization = "ABC",
            Repository = "DEF",
        ...
        var issue = new GitHubIssue(knownIssueValidationMessage.IssueId, body: "issue_body",
            repositoryWithOwner: $"{knownIssueValidationMessage.Organization}/{knownIssueValidationMessage.Repository}");
        var knownIssues = new List<KnownIssue> {new(issue, new List<string>(){""}, KnownIssueType.Test, new KnownIssueOptions())};

        await using TestData testData = await TestData.Default
            .WithIssue(MockGithubIssue(bodyWithKnownIssueAndWithBuild, knownIssueValidationMessage.IssueId))
            .WithBuildResultAnalysis(MockBuildResultAnalysis(knownIssues))
            .BuildAsync();

        await testData.Processor.ValidateKnownIssue(knownIssueValidationMessage, CancellationToken.None);
...
```";
        ...
        {
            new("PowerShell exited with code", 123456, "1")
        };
...
<!--Known issue error report end -->";
    ...
    private KnownIssueValidationMessage TestKnownIssueValidationMessage()
    {
        return new KnownIssueValidationMessage
        {
