...
internal class VmrBackflowTest : VmrCodeFlowTests
{
    ...
    public async Task BackflowingDependenciesTest()
    {
        ...
        var updatedGlobalJson = await File.ReadAllTextAsync(VmrArcade / VersionFiles.GlobalJson);
        await File.WriteAllTextAsync(VmrArcade / VersionFiles.GlobalJson, updatedGlobalJson.Replace("9.0.100", "9.0.200"));
        ...
        JObject globalJson = await dependencyFileManager.ReadGlobalJsonAsync(ProductRepoPath, branchName + "-pr", lookInSrcArcade: false);
