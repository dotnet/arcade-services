...
internal class ForwardFlowTests : CodeFlowTests
{
    ...
    // Tests that the source and target repos are restored to their original branches after forward flow completes
    [Test]
    public async Task ForwardFlowRestoresOriginalBranchTest()
    {
        await EnsureTestRepoIsInitialized();

        // Make a simple change to flow (without touching Version.Details.xml which has the Source tag)
        await File.WriteAllTextAsync(_productRepoFilePath, "Content for branch restoration test");
        await GitOperations.CommitAll(ProductRepoPath, "Add content for branch restoration test");

        // Get the original branches before forward flow
        var originalProductRepoBranch = (await GitOperations.ExecuteGitCommand(ProductRepoPath, "rev-parse", "--abbrev-ref", "HEAD")).StandardOutput.Trim();
        var originalVmrBranch = (await GitOperations.ExecuteGitCommand(VmrPath, "rev-parse", "--abbrev-ref", "HEAD")).StandardOutput.Trim();

        originalProductRepoBranch.Should().Be("main", "Product repo should start on main branch");
        originalVmrBranch.Should().Be("main", "VMR should start on main branch");

        // Perform forward flow
        await CallDarcForwardflow();

        // Verify both repos are restored to their original branches
        var currentProductRepoBranch = (await GitOperations.ExecuteGitCommand(ProductRepoPath, "rev-parse", "--abbrev-ref", "HEAD")).StandardOutput.Trim();
        var currentVmrBranch = (await GitOperations.ExecuteGitCommand(VmrPath, "rev-parse", "--abbrev-ref", "HEAD")).StandardOutput.Trim();

        currentProductRepoBranch.Should().Be(originalProductRepoBranch, "Product repo should be restored to its original branch");
        currentVmrBranch.Should().Be(originalVmrBranch, "VMR should be restored to its original branch");
    }

    // Tests that the repos are restored even when forward flow throws an exception
    [Test]
    public async Task ForwardFlowRestoresOriginalBranchOnErrorTest()
    {
        await EnsureTestRepoIsInitialized();

        // Checkout the VMR to a specific branch
        await GitOperations.CreateBranch(VmrPath, "test-branch");
        await GitOperations.Checkout(VmrPath, "test-branch");

        // Make a change in the product repo
        await File.WriteAllTextAsync(_productRepoFilePath, "New content");
        await GitOperations.CommitAll(ProductRepoPath, "New content");
        ...
        originalProductRepoBranch.Should().Be("main", "Product repo should start on main branch");
        originalVmrBranch.Should().Be("test-branch", "VMR should start on test-branch");
