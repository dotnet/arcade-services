...
public class CoherencyUpdateResolverTests
{
    ...
    /// <summary>
    /// Verifies that when the coherent parent's dependency list contains the same version/commit/repo
    /// for the child, the method identifies it as already up to date and produces no updates.
    /// Inputs:
    ///  - Parent "A" and child "B" (B.CoherentParentDependencyName = "A"), B not pinned.
    ///  - Remote.GetDependenciesAsync(parent.RepoUri, parent.Commit, name:null) returns "B" matching exactly.
    /// Expected:
    ///  - Returns an empty list.
    ///  - IRemoteFactory.CreateRemoteAsync and IRemote.GetDependenciesAsync are called once.
    /// </summary>
    [Test]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    [Category("auto-generated")]
    public async Task GetRequiredCoherencyUpdatesAsync_AlreadyUpToDate_RemoteCalledButNoUpdates()
    {
        // Arrange
        var barClient = new Mock<IBasicBarClient>(MockBehavior.Strict);
        ...
        var dependencies = new List<DependencyDetail> { parent, child };

        remoteFactory
            .Setup(f => f.CreateRemoteAsync(parent.RepoUri))
            .ReturnsAsync(remote.Object);

        // Explicitly specify all optional parameters in the setup.
        remote
            .Setup(r => r.GetDependenciesAsync(parent.RepoUri, parent.Commit, It.IsAny<string>()))
        ...
        // Assert
        result.Should().BeEmpty();
        remoteFactory.Verify(f => f.CreateRemoteAsync(parent.RepoUri), Times.Once);
        remote.Verify(r => r.GetDependenciesAsync(parent.RepoUri, parent.Commit, It.IsAny<string>()), Times.Once);
    }

    /// <summary>
    /// Validates that when the coherent parent's dependency list contains a different version/commit/repo for the child,
    /// an update is produced with fields sourced from the coherent dependency and Locations resolved via DisambiguateAssetsAsync.
    /// Inputs:
    ///  - Parent "A" and child "B" (B.CoherentParentDependencyName = "A"), B not pinned.
    ///  - Remote.GetDependenciesAsync returns "B" with different Version/RepoUri/Commit.
    ///  - BAR GetBuildsAsync returns no builds, leading to null asset and thus null Locations on the updated dependency.
    /// Expected:
    ///  - One DependencyUpdate returned.
    ///  - From == original child.
    ///  - To has Name/Version/RepoUri/Commit from the coherent dependency and Locations == null.
    /// </summary>
    [Test]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    [Category("auto-generated")]
    public async Task GetRequiredCoherencyUpdatesAsync_CpdDifferentVersionAndRepo_ProducesDependencyUpdate()
    {
        // Arrange
        var barClient = new Mock<IBasicBarClient>(MockBehavior.Strict);
        ...
        var child = new DependencyDetail
        {
            Name = "B",
            Version = "0.9.0",
            RepoUri = "https://repo/old-b",
            Commit = "old-sha-b",
            CoherentParentDependencyName = "A",
            Pinned = false
        };

        var cpdDependency = new DependencyDetail
        {
            Name = "B",
            Version = "1.2.3",
            RepoUri = "https://repo/new-b",
            Commit = "new-sha-b",
            Pinned = false
        };
        ...
        remote
            .Setup(r => r.GetDependenciesAsync(parent.RepoUri, parent.Commit, It.IsAny<string>()))
            .ReturnsAsync(new List<DependencyDetail> { cpdDependency });

        // Disambiguation path: no builds -> coherentAsset == null
        barClient
            .Setup(b => b.GetBuildsAsync(cpdDependency.RepoUri, cpdDependency.Commit))
            .ReturnsAsync(Array.Empty<Build>());
        ...
        var update = result.Single();
        update.From.Should().NotBeNull();
        update.To.Should().NotBeNull();

        update.From.Name.Should().Be("B");
        update.From.Version.Should().Be("0.9.0");
        update.From.RepoUri.Should().Be("https://repo/old-b");
        update.From.Commit.Should().Be("old-sha-b");

        update.To.Name.Should().Be("B");
        update.To.Version.Should().Be("1.2.3");
        update.To.RepoUri.Should().Be("https://repo/new-b");
        update.To.Commit.Should().Be("new-sha-b");
        update.To.Locations.Should().BeNull();

        remoteFactory.Verify(f => f.CreateRemoteAsync(parent.RepoUri), Times.Once);
        remote.Verify(r => r.GetDependenciesAsync(parent.RepoUri, parent.Commit, It.IsAny<string>()), Times.Once);
        barClient.Verify(b => b.GetBuildsAsync(cpdDependency.RepoUri, cpdDependency.Commit), Times.Once);
    }

    /// <summary>
    /// Ensures no updates are produced when the assets collection is empty.
    /// Inputs:
    ///  - assets: empty collection.
    ///  - dependencies: any.
    /// Expected:
    ///  - Empty result (no DependencyUpdate entries).
    /// </summary>
    ...
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    [Category("auto-generated")]
    public void GetRequiredNonCoherencyUpdates_EmptyAssets_ReturnsEmpty()
    {
        // Arrange
        var bar = new Mock<IBasicBarClient>(MockBehavior.Strict).Object;
        var remoteFactory = new Mock<IRemoteFactory>(MockBehavior.Strict).Object;
        ...
        var assets = Array.Empty<AssetData>();
        var dependencies = new[]
        {
            new DependencyDetail { Name = "X", Version = "0.1", RepoUri = "r", Commit = "c" }
        };
    ...
    /// <summary>
    /// Verifies that when asset locations are null, the resulting 'To' dependency has Locations set to null.
    /// Inputs:
    ///  - Dependency matching asset by name.
    ///  - Asset with Locations = null.
    /// Expected:
    ///  - One update is produced with To.Locations == null.
    /// </summary>
    [Test]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    [Category("auto-generated")]
    public void GetRequiredNonCoherencyUpdates_NullAssetLocations_ToLocationsNull()
    {
        // Arrange
        var bar = new Mock<IBasicBarClient>(MockBehavior.Strict).Object;
        var remoteFactory = new Mock<IRemoteFactory>(MockBehavior.Strict).Object;
        ...
        var dependencies = new[]
        {
            new DependencyDetail { Name = "Pkg", Version = "0.1", RepoUri = "r", Commit = "c" }
        };
        var assets = new[]
        {
            new AssetData(false) { Name = "Pkg", Version = "9.9.9", Locations = null }
        };
        ...
        // Assert
        updates.Should().HaveCount(1);
        updates[0].To.Locations.Should().BeNull();
    }
}
