...
public class LocalGitRepoTests
{
    ...
    /// <summary>
    /// Verifies that when the underlying process manager returns a canceled Task for the provided token,
    /// LocalGitRepo.ExecuteGitCommand throws OperationCanceledException.
    /// Inputs:
    ///  - args: a simple arguments array.
    ///  - cancellationToken: a specific token used to cancel the task.
    /// Expected:
    ///  - OperationCanceledException is thrown.
    ///  - IProcessManager.ExecuteGit is invoked exactly once with the same args and token.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public void ExecuteGitCommand_WhenProcessManagerTaskIsCanceled_ThrowsOperationCanceled()
    {
        // Arrange
        var localGitClientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);

        var args = new[] { "fetch", "--all" };
        using var cts = new CancellationTokenSource();
        var token = cts.Token;
        ...
        processManagerMock
            .Setup(m => m.ExecuteGit(
                ...
                It.Is<Dictionary<string, string>>(d => d == null),
                It.Is<CancellationToken>(t => t.Equals(token))))
            .Returns(Task.FromCanceled<ProcessExecutionResult>(token));

        var sut = new LocalGitRepo(default, localGitClientMock.Object, processManagerMock.Object);

        // Act + Assert
        Assert.ThrowsAsync<OperationCanceledException>(async () => await sut.ExecuteGitCommand(args, token));
        processManagerMock.Verify(m => m.ExecuteGit(
            It.IsAny<string>(),
            It.Is<string[]>(a => ReferenceEquals(a, args)),
            It.Is<Dictionary<string, string>>(d => d == null),
    ...
    /// <summary>
    /// Verifies that AddRemoteIfMissingAsync forwards the repository URL and the exact CancellationToken
    /// to ILocalGitClient.AddRemoteIfMissingAsync and returns the same remote name provided by the client.
    /// Inputs (parameterized):
    ///  - repoUrl: variants including empty, whitespace, special/unicode characters, and typical URLs.
    ///  - useSpecificToken: whether a specific CancellationToken instance is passed.
    ///  - preCanceled: whether the specific token is canceled before the call.
    ///  - expectedRemoteName: the value returned by the client to be propagated.
    /// Expected:
    ///  - ILocalGitClient.AddRemoteIfMissingAsync is invoked exactly once with:
    ///      * any repo path string,
    ///      * the exact repoUrl string,
    ///      * the exact CancellationToken instance as supplied.
    ///  - The returned string equals expectedRemoteName.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [TestCaseSource(nameof(AddRemoteIfMissingAsync_Forwarding_Cases))]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task AddRemoteIfMissingAsync_ForwardsArgsAndToken_ReturnsClientResult(
        string repoUrl,
        bool useSpecificToken,
        bool preCanceled,
        string expectedRemoteName)
    {
        ...
        localGitClientMock
            .Setup(m => m.AddRemoteIfMissingAsync(
                It.IsAny<string>(),
                It.Is<string>(u => ReferenceEquals(u, repoUrl) || u == repoUrl),
                It.Is<CancellationToken>(t => t.Equals(token))))
            .ReturnsAsync(expectedRemoteName);

        var sut = new LocalGitRepo(default, localGitClientMock.Object, processManagerMock.Object);

        // Act
        var actual = await sut.AddRemoteIfMissingAsync(repoUrl, token);

        // Assert
        actual.Should().Be(expectedRemoteName);
        localGitClientMock.Verify(m => m.AddRemoteIfMissingAsync(
            It.IsAny<string>(),
            It.Is<string>(u => ReferenceEquals(u, repoUrl) || u == repoUrl),
            It.Is<CancellationToken>(t => t.Equals(token))), Times.Once);
        localGitClientMock.VerifyNoOtherCalls();
    ...
    /// <summary>
    /// Ensures that when no CancellationToken is provided, AddRemoteIfMissingAsync uses the default token
    /// (non-cancelable, not requested) and returns the exact value from the client.
    /// Inputs:
    ///  - repoUrl: a typical remote URL string.
    /// Expected:
    ///  - ILocalGitClient.AddRemoteIfMissingAsync receives a default, non-cancelable CancellationToken.
    ///  - The returned value matches the client's result.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task AddRemoteIfMissingAsync_WithoutToken_UsesDefaultAndReturnsClientValue()
    {
        // Arrange
        var localGitClientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);

        var repoUrl = "https://github.com/org/repo.git";
        var expectedRemote = "origin";

        localGitClientMock
            .Setup(m => m.AddRemoteIfMissingAsync(
                It.IsAny<string>(),
                It.Is<string>(u => u == repoUrl),
                It.Is<CancellationToken>(ct => !ct.CanBeCanceled && !ct.IsCancellationRequested)))
            .ReturnsAsync(expectedRemote);

        var sut = new LocalGitRepo(default, localGitClientMock.Object, processManagerMock.Object);

        // Act
        var actual = await sut.AddRemoteIfMissingAsync(repoUrl);

        // Assert
        actual.Should().Be(expectedRemote);
        localGitClientMock.Verify(m => m.AddRemoteIfMissingAsync(
            It.IsAny<string>(),
            It.Is<string>(u => u == repoUrl),
            It.Is<CancellationToken>(ct => !ct.CanBeCanceled && !ct.IsCancellationRequested)), Times.Once);
        localGitClientMock.VerifyNoOtherCalls();
        processManagerMock.VerifyNoOtherCalls();
    }

    /// <summary>
    /// Validates that exceptions thrown by ILocalGitClient.AddRemoteIfMissingAsync are propagated unchanged.
    /// Inputs:
    ///  - repoUrl: "bad://url"
    ///  - token: default CancellationToken.
    /// Expected:
    ///  - LocalGitRepo.AddRemoteIfMissingAsync throws the same InvalidOperationException instance.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task AddRemoteIfMissingAsync_WhenClientThrows_ExceptionPropagates()
    {
        // Arrange
        var localGitClientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);
        ...
        try
        {
            await sut.AddRemoteIfMissingAsync(repoUrl, token);
        }
    ...
    /// <summary>
    /// Verifies that when the underlying client returns a canceled task,
    /// LocalGitRepo.AddRemoteIfMissingAsync throws OperationCanceledException.
    /// Inputs:
    ///  - repoUrl: "https://example.com/repo.git"
    ///  - token: a pre-canceled CancellationToken.
    /// Expected:
    ///  - OperationCanceledException is thrown by LocalGitRepo.AddRemoteIfMissingAsync.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task AddRemoteIfMissingAsync_WhenClientTaskIsCanceled_ThrowsOperationCanceled()
    {
        // Arrange
        var localGitClientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);

        var repoUrl = "https://example.com/repo.git";
        var cts = new CancellationTokenSource();
        cts.Cancel();
        var token = cts.Token;
        ...
        localGitClientMock
            .Setup(m => m.AddRemoteIfMissingAsync(
                ...
                It.Is<string>(u => u == repoUrl),
                It.Is<CancellationToken>(t => t.Equals(token))))
            .Returns(Task.FromCanceled<string>(token));
    ...
    /// <summary>
    /// Validates that if the client returns null (despite non-nullable signature),
    /// LocalGitRepo.AddRemoteIfMissingAsync propagates the null value.
    /// Inputs:
    ///  - repoUrl: empty string (edge-case).
    /// Expected:
    ...
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task AddRemoteIfMissingAsync_UnderlyingReturnsNull_PropagatesNull()
    {
        // Arrange
        var localGitClientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);
    ...
    private static IEnumerable<object[]> AddRemoteIfMissingAsync_Forwarding_Cases()
    {
        yield return new object[] { "", false, false, "origin" };
        yield return new object[] { " ", true, false, "upstream" };
        yield return new object[] { "\t\n", true, true, "remote-tab-newline" };
        yield return new object[] { "https://github.com/org/repo.git", false, false, "r" };
        yield return new object[] { "ssh://git@github.com:org/repo", true, false, "git-remote" };
        yield return new object[] { "特殊/字符\t\n🌟", false, false, "unicode" };
        yield return new object[] { new string('a', 1024), true, false, "long-remote" };
    }
    ...
    /// all parameters to ILocalGitClient.BlameLineAsync and returns the expected SHA.
    /// Inputs:
    ///  - Various filePath and blameFromCommit combinations, including empty, whitespace, special characters, and null.
    ///  - A non-null predicate delegate.
    /// Expected:
    ///  - The exact same parameters are passed through to the client, and the returned SHA matches the mock setup.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [TestCase("a.txt", null, "sha-a")]
    [TestCase("", null, "sha-empty")]
    [TestCase(" ", "", "sha-ws")]
    [TestCase("/unix/path/файл.txt", " ", "sha-unicode")]
    [TestCase("path/with:invalid*chars?.txt", "deadbeefdeadbeefdeadbeefdeadbeefdeadbeef", "sha-longsha")]
    [TestCase("./relative/../path.txt", "abc123", "sha-abc123")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task BlameLineAsync_ValidInputs_DelegatesAndReturnsExpectedSha(string filePath, string blameFromCommit, string expectedSha)
    {
        // Arrange
        var localGitClientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);
    ...
    /// <summary>
    /// Verifies that LocalGitRepo.HasWorkingTreeChangesAsync forwards the call to ILocalGitClient.HasWorkingTreeChangesAsync
    /// and returns the exact boolean value produced by the client.
    /// Inputs:
    ///  - clientResult: boolean values true and false.
    /// Expected:
    ///  - The method returns the same boolean as the client and calls the client exactly once with any repo path.
    /// </summary>
    [TestCase(true)]
    [TestCase(false)]
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task HasWorkingTreeChangesAsync_DelegatesToClient_ReturnsClientResult(bool clientResult)
    {
        // Arrange
        var clientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var procMgrMock = new Mock<IProcessManager>(MockBehavior.Strict);

        clientMock
            .Setup(c => c.HasWorkingTreeChangesAsync(It.IsAny<string>()))
            .ReturnsAsync(clientResult);

        var sut = new LocalGitRepo(default, clientMock.Object, procMgrMock.Object);

        // Act
        var actual = await sut.HasWorkingTreeChangesAsync();

        // Assert
        actual.Should().Be(clientResult);
        clientMock.Verify(c => c.HasWorkingTreeChangesAsync(It.IsAny<string>()), Times.Once);
        clientMock.VerifyNoOtherCalls();
        procMgrMock.VerifyNoOtherCalls();
    }

    /// <summary>
    /// Ensures that exceptions from ILocalGitClient.HasWorkingTreeChangesAsync are propagated unchanged by LocalGitRepo.HasWorkingTreeChangesAsync.
    /// Inputs:
    ///  - ILocalGitClient.HasWorkingTreeChangesAsync configured to throw InvalidOperationException.
    /// Expected:
    ///  - LocalGitRepo.HasWorkingTreeChangesAsync throws InvalidOperationException.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task HasWorkingTreeChangesAsync_WhenClientThrows_ExceptionPropagates()
    {
        // Arrange
        var clientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var procMgrMock = new Mock<IProcessManager>(MockBehavior.Strict);
        ...
        // Act
        Func<Task> act = async () => await sut.HasWorkingTreeChangesAsync();
    ...
    /// <summary>
    /// Verifies that exceptions thrown by ILocalGitClient.HasStagedChangesAsync are propagated by LocalGitRepo.HasStagedChangesAsync.
    /// Inputs:
    ///  - ILocalGitClient configured to throw InvalidOperationException with a specific message.
    /// Expected:
    ...
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task HasStagedChangesAsync_WhenClientThrows_ExceptionPropagates()
    {
        // Arrange
        var expectedMessage = "client failure";
        var gitClientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        gitClientMock
            .Setup(m => m.HasStagedChangesAsync(It.IsAny<string>()))
        ...
        var repoPath = new NativePath("repo-test");
        var sut = new LocalGitRepo(repoPath, gitClientMock.Object, processManagerMock.Object);

        // Act
        InvalidOperationException caught = null;
        try
        {
            await sut.HasStagedChangesAsync();
        }
        catch (InvalidOperationException ex)
        {
            caught = ex;
    ...
    /// <summary>
    /// Verifies that CheckoutAsync forwards the repo path and the exact ref string to ILocalGitClient.CheckoutAsync.
    /// Inputs:
    ///  - refToCheckout: a variety of strings including empty, whitespace, refs-style, special, and long values.
    /// Expected:
    ///  - ILocalGitClient.CheckoutAsync is invoked exactly once with any repo path and the exact refToCheckout value.
    ///  - The call completes successfully without throwing.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [TestCaseSource(nameof(CheckoutRefs))]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task CheckoutAsync_ForwardsRefToClient_InvokesOnce(string refToCheckout)
    {
        // Arrange
        var clientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var procMgrMock = new Mock<IProcessManager>(MockBehavior.Strict);
        ...
        clientMock
            .Setup(c => c.CheckoutAsync(
                It.IsAny<string>(),
                It.Is<string>(r => r == refToCheckout)))
            .Callback<string, string>((repoPath, r) =>
            {
        ...
        // Act
        await sut.CheckoutAsync(refToCheckout);

        // Assert
        capturedRef.Should().Be(refToCheckout);
        clientMock.Verify(c => c.CheckoutAsync(
                It.IsAny<string>(),
                It.Is<string>(r => r == refToCheckout)),
            Times.Once);
        clientMock.VerifyNoOtherCalls();
    ...
    /// <summary>
    /// Ensures that exceptions thrown by ILocalGitClient.CheckoutAsync are propagated unchanged by LocalGitRepo.CheckoutAsync.
    /// Inputs:
    ///  - refToCheckout: "boom-branch"
    /// Expected:
    ///  - InvalidOperationException thrown with the configured message from the client.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task CheckoutAsync_WhenClientThrows_ExceptionPropagates()
    {
        // Arrange
        var clientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var procMgrMock = new Mock<IProcessManager>(MockBehavior.Strict);
        ...
        var expected = new InvalidOperationException("boom");
        var refToCheckout = "boom-branch";

        clientMock
            .Setup(c => c.CheckoutAsync(It.IsAny<string>(), It.Is<string>(r => r == refToCheckout)))
            .ThrowsAsync(expected);

        var sut = new LocalGitRepo(default, clientMock.Object, procMgrMock.Object);

        // Act + Assert
        var ex = Assert.ThrowsAsync<InvalidOperationException>(async () => await sut.CheckoutAsync(refToCheckout));
        ex.Should().BeSameAs(expected);
        clientMock.Verify(c => c.CheckoutAsync(It.IsAny<string>(), It.Is<string>(r => r == refToCheckout)), Times.Once);
        clientMock.VerifyNoOtherCalls();
        procMgrMock.VerifyNoOtherCalls();
    }

    /// <summary>
    /// Verifies that when the underlying ILocalGitClient.CheckoutAsync returns a canceled task,
    /// LocalGitRepo.CheckoutAsync throws OperationCanceledException.
    /// Inputs:
    ///  - refToCheckout: "feature/cancel"
    /// Expected:
    ...
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task CheckoutAsync_WhenClientTaskIsCanceled_ThrowsOperationCanceled()
    {
        // Arrange
        var clientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var procMgrMock = new Mock<IProcessManager>(MockBehavior.Strict);

        var refToCheckout = "feature/cancel";
        var cts = new CancellationTokenSource();
        cts.Cancel();
        ...
        // Act + Assert
        Assert.ThrowsAsync<OperationCanceledException>(async () => await sut.CheckoutAsync(refToCheckout));
        clientMock.Verify(c => c.CheckoutAsync(It.IsAny<string>(), It.Is<string>(r => r == refToCheckout)), Times.Once);
        clientMock.VerifyNoOtherCalls();
        procMgrMock.VerifyNoOtherCalls();
    }

    private static IEnumerable CheckoutRefs()
    {
        yield return "main";
        yield return "";
        yield return " ";
        yield return "HEAD";
        yield return "refs/heads/feature";
        yield return "origin/dev";
        yield return "v1.0.0";
        yield return "0123456789abcdef0123456789abcdef01234567";
        yield return "feature/with spaces and 🐱";
        yield return new string('a', 256);
    }

    /// <summary>
    /// Verifies that CommitAsync forwards all parameters (message, allowEmpty, author, cancellationToken)
    /// to ILocalGitClient.CommitAsync exactly once and completes successfully.
    /// Inputs (parameterized):
    ///  - message: "", whitespace, long, and special-character strings.
    ///  - allowEmpty: true/false.
    ///  - hasAuthor: whether to pass a non-null author tuple.
    ///  - authorName/authorEmail: used when hasAuthor is true.
    ///  - useSpecificToken: whether to pass a specific CancellationToken instance.
    ///  - preCanceled: whether that specific token is already canceled before invocation.
    /// Expected:
    ///  - ILocalGitClient.CommitAsync is invoked once with the exact forwarded parameters.
    ///  - No exception is thrown.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [TestCaseSource(nameof(CommitAsync_Forwarding_Cases))]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task CommitAsync_ForwardsAllParameters_ToClient(
        string message,
        bool allowEmpty,
        bool hasAuthor,
        string authorName,
        ...
        bool preCanceled)
    {
        ...
        (string Name, string Email)? expectedAuthor = hasAuthor ? (authorName, authorEmail) : ((string, string)?)null;
        ...
        localGitClientMock
            .Setup(m => m.CommitAsync(
                It.IsAny<string>(),
                It.Is<string>(msg => ReferenceEquals(msg, message)),
                It.Is<bool>(ae => ae == allowEmpty),
                It.Is<(string Name, string Email)?>(a => AuthorsMatch(a, expectedAuthor)),
                It.Is<CancellationToken>(t => t.Equals(token))))
            .Returns(Task.CompletedTask);
        ...
        // Act
        await repo.CommitAsync(message, allowEmpty, expectedAuthor, token);

        // Assert
        localGitClientMock.Verify(m => m.CommitAsync(
                It.IsAny<string>(),
                It.Is<string>(msg => ReferenceEquals(msg, message)),
                It.Is<bool>(ae => ae == allowEmpty),
                It.Is<(string Name, string Email)?>(a => AuthorsMatch(a, expectedAuthor)),
    ...
    /// <summary>
    /// Ensures exceptions thrown by ILocalGitClient.CommitAsync are propagated unchanged.
    /// Inputs:
    ///  - message: "boom"
    ///  - allowEmpty: false
    ///  - author: null
    ///  - cancellationToken: default
    /// Expected:
    ///  - CommitAsync throws InvalidOperationException.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public void CommitAsync_WhenClientThrows_ExceptionPropagates()
    {
        // Arrange
        var localGitClientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);
        ...
        var thrown = Assert.ThrowsAsync<InvalidOperationException>(async () =>
            await repo.CommitAsync(message, allowEmpty, author, token));
    ...
    /// <summary>
    /// Verifies that when the underlying client returns a canceled task for the provided token,
    /// LocalGitRepo.CommitAsync throws OperationCanceledException.
    /// Inputs:
    ///  - message: "msg"
    ///  - allowEmpty: true
    ///  - author: ("Name", "email@example.com")
    ///  - cancellationToken: pre-canceled token
    /// Expected:
    ///  - CommitAsync throws OperationCanceledException.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public void CommitAsync_WhenClientTaskIsCanceled_ThrowsOperationCanceled()
    {
        // Arrange
        var localGitClientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);
        ...
        var message = "msg";
        var allowEmpty = true;
        (string Name, string Email)? author = ("Name", "email@example.com");
        ...
        localGitClientMock
            .Setup(m => m.CommitAsync(
                ...
                It.Is<string>(msg => ReferenceEquals(msg, message)),
                It.Is<bool>(ae => ae == allowEmpty),
                It.Is<(string Name, string Email)?>(a => AuthorsMatch(a, author)),
                It.Is<CancellationToken>(t => t.Equals(token))))
            .Returns(Task.FromCanceled(token));
    ...
    private static IEnumerable CommitAsync_Forwarding_Cases()
    {
        // message, allowEmpty, hasAuthor, authorName, authorEmail, useSpecificToken, preCanceled
        yield return new object[] { "", false, false, "", "", false, false };
        yield return new object[] { " ", true, true, "John Doe", "john@example.com", false, false };
        yield return new object[]
        {
            "feat(scope): special !@#$%^&*()_+-=[]{}|;':,.<>/?`~\t\n", false, true, "Äuthor 名", "äuthor@example.com", true, false
        };
        yield return new object[] { new string('a', 1024), true, false, "", "", true, false };
        yield return new object[] { "regular message", false, true, "Bot", "bot@contoso.com", true, true };
    }

    /// <summary>
    /// Verifies that CreateBranchAsync forwards the repository path, the exact branch name,
    /// and the overwriteExistingBranch flag to ILocalGitClient.CreateBranchAsync.
    /// Inputs:
    ///  - branchName: various strings including empty, whitespace, long, special, path-like, and unicode.
    ///  - overwrite: true/false flag.
    /// Expected:
    ///  - ILocalGitClient.CreateBranchAsync is invoked exactly once with:
    ///      * any non-null repo path string,
    ///      * the exact branchName instance,
    ///      * the exact overwrite value.
    ...
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task CreateBranchAsync_ForwardsParameters_ToLocalGitClient(string branchName, bool overwrite)
    {
        // Arrange
        var client = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var proc = new Mock<IProcessManager>(MockBehavior.Strict);

        client
            .Setup(c => c.CreateBranchAsync(
                It.IsAny<string>(),
                It.Is<string>(bn => ReferenceEquals(bn, branchName)),
                It.Is<bool>(o => o == overwrite)))
            .Returns(Task.CompletedTask);

        var repo = new LocalGitRepo(default, client.Object, proc.Object);

        // Act
        Func<Task> act = async () => await repo.CreateBranchAsync(branchName, overwrite);
        ...
        client.Verify(c => c.CreateBranchAsync(
            It.IsAny<string>(),
            It.Is<string>(bn => ReferenceEquals(bn, branchName)),
            It.Is<bool>(o => o == overwrite)), Times.Once);
        client.VerifyNoOtherCalls();
        proc.VerifyNoOtherCalls();
    }

    /// <summary>
    /// Ensures that when the overwriteExistingBranch parameter is omitted,
    /// LocalGitRepo.CreateBranchAsync passes overwrite = false to ILocalGitClient.CreateBranchAsync.
    /// Inputs:
    ///  - branchName: a representative branch name.
    /// Expected:
    ///  - ILocalGitClient.CreateBranchAsync is called once with overwriteExistingBranch = false.
    ///  - The call completes without throwing.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task CreateBranchAsync_OmittedOverwrite_UsesFalse()
    {
        // Arrange
        var client = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var proc = new Mock<IProcessManager>(MockBehavior.Strict);
        var branchName = "feature/omitted-flag";

        client
            .Setup(c => c.CreateBranchAsync(
                It.IsAny<string>(),
                It.Is<string>(bn => bn == branchName),
                It.Is<bool>(o => o == false)))
            .Returns(Task.CompletedTask);

        var repo = new LocalGitRepo(default, client.Object, proc.Object);

        // Act
        Func<Task> act = async () => await repo.CreateBranchAsync(branchName);
        ...
        client.Verify(c => c.CreateBranchAsync(
            It.IsAny<string>(),
            It.Is<string>(bn => bn == branchName),
            It.Is<bool>(o => o == false)), Times.Once);
        client.VerifyNoOtherCalls();
        proc.VerifyNoOtherCalls();
    }

    /// <summary>
    /// Verifies that exceptions thrown by ILocalGitClient.CreateBranchAsync are propagated unchanged
    /// by LocalGitRepo.CreateBranchAsync.
    /// Inputs:
    ///  - branchName: a representative branch name that triggers an exception.
    ///  - overwrite: either true or false.
    /// Expected:
    ///  - LocalGitRepo.CreateBranchAsync throws InvalidOperationException with the configured message.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [TestCase("bad/branch", true)]
    [TestCase("bad/branch", false)]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task CreateBranchAsync_WhenClientThrows_ExceptionIsPropagated(string branchName, bool overwrite)
    {
        // Arrange
        var client = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var proc = new Mock<IProcessManager>(MockBehavior.Strict);
        var expectedMessage = "boom-create-branch";
    ...
    private static IEnumerable CreateBranchAsync_Cases()
    {
        // assorted branch names and overwrite flags
        yield return new object[] { "main", false };
        yield return new object[] { "main", true };
        yield return new object[] { "", false };
        yield return new object[] { " ", true };
        yield return new object[] { "feature/new", false };
        yield return new object[] { "refs/heads/release/1.0", true };
        yield return new object[] { "bugfix/1234-fix-issue", false };
        yield return new object[] { "特殊/分支", true };
        yield return new object[] { "branch with spaces", false };
        yield return new object[] { "weird!@#$%^&*()_+|<>?;,'\"", true };
        yield return new object[] { new string('a', 256), false };
    }

    /// <summary>
    /// Verifies that when no CancellationToken is provided, LocalGitRepo.GetGitCommitAsync
    /// forwards a default, non-cancelable token to ILocalGitClient and returns the client's value.
    /// Inputs:
    ///  - No token passed (uses default).
    /// Expected:
    ///  - The captured token is default (non-cancelable, not requested).
    ///  - The returned commit string equals the mocked value.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task GetGitCommitAsync_NoToken_UsesDefaultTokenAndReturnsValue()
    {
        // Arrange
        var expectedCommit = "default-token-sha";
        var localGitClientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);
        ...
        // Act
        var result = await sut.GetGitCommitAsync();

        // Assert
        result.Should().Be(expectedCommit);
        capturedToken.CanBeCanceled.Should().Be(false);
        capturedToken.IsCancellationRequested.Should().Be(false);
        localGitClientMock.Verify(m => m.GetGitCommitAsync(It.IsAny<string>(), It.IsAny<CancellationToken>()), Times.Once);
        processManagerMock.VerifyNoOtherCalls();
    }

    /// <summary>
    /// Verifies that when the underlying client returns a canceled Task, LocalGitRepo.GetGitCommitAsync
    /// throws OperationCanceledException and forwards the exact CancellationToken instance.
    /// Inputs:
    ///  - A pre-canceled CancellationToken.
    /// Expected:
    ///  - OperationCanceledException is thrown.
    ///  - ILocalGitClient.GetGitCommitAsync is invoked once with the same token instance.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task GetGitCommitAsync_WhenClientTaskIsCanceled_ThrowsOperationCanceled()
    {
        // Arrange
        var localGitClientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);
        ...
        // Act
        Func<Task> act = async () => await sut.GetGitCommitAsync(token);
    ...
    /// <summary>
    /// Ensures that exceptions thrown by ILocalGitClient.GetGitSubmodulesAsync are propagated unchanged.
    /// Inputs:
    ///  - Specific commit strings causing the client to throw InvalidOperationException with a known message.
    /// Expected:
    ///  - LocalGitRepo.GetGitSubmodulesAsync throws InvalidOperationException with the same message.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task GetGitSubmodulesAsync_ClientThrows_ExceptionPropagates()
    {
        // Arrange
        var localGitClientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);

        const string commit = "deadbeefcafebabe";
        const string errorMessage = "client failure";

        localGitClientMock
            .Setup(m => m.GetGitSubmodulesAsync(
                It.IsAny<string>(),
                It.Is<string>(c => c == commit)))
            .ThrowsAsync(new InvalidOperationException(errorMessage));

        var sut = new LocalGitRepo(default(NativePath), localGitClientMock.Object, processManagerMock.Object);

        // Act
        Func<Task> act = () => sut.GetGitSubmodulesAsync(commit);
    ...
    /// <summary>
    /// Verifies that GetShaForRefAsync forwards the repository path and the provided gitRef
    /// to ILocalGitClient.GetShaForRefAsync and returns the exact SHA produced by the client.
    /// Inputs:
    ///  - gitRef variations including null (default behavior/HEAD in client), empty, whitespace, unicode, long, and special characters.
    /// Expected:
    ///  - ILocalGitClient.GetShaForRefAsync is called once with any repo path and the exact gitRef argument.
    ///  - The returned string equals the mocked SHA value from the client.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [TestCaseSource(nameof(GetShaForRefAsync_GitRefInputs))]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task GetShaForRefAsync_VariousGitRefs_ForwardsAndReturnsClientResult(string gitRef)
    {
        // Arrange
        var client = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var proc = new Mock<IProcessManager>(MockBehavior.Strict);

        var expectedSha = $"sha-for-{gitRef ?? "null"}";
        client
            .Setup(c => c.GetShaForRefAsync(
                It.IsAny<string>(),
                It.Is<string>(gr => gr == gitRef)))
            .ReturnsAsync(expectedSha);

        var sut = new LocalGitRepo(default(NativePath), client.Object, proc.Object);

        // Act
        var actual = await sut.GetShaForRefAsync(gitRef);

        // Assert
        actual.Should().Be(expectedSha);
        client.Verify(c => c.GetShaForRefAsync(It.IsAny<string>(), It.Is<string>(gr => gr == gitRef)), Times.Once);
        client.VerifyNoOtherCalls();
        proc.VerifyNoOtherCalls();
    }

    /// <summary>
    /// Ensures that when no argument is provided, GetShaForRefAsync passes null for gitRef
    /// to the client (letting the client resolve to HEAD) and returns the client's result.
    /// Inputs:
    ///  - No gitRef argument (uses default).
    /// Expected:
    ///  - gitRef forwarded as null to ILocalGitClient.GetShaForRefAsync.
    ///  - The returned SHA matches the client's value.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task GetShaForRefAsync_NoArgument_PassesNullAndReturnsClientValue()
    {
        // Arrange
        var client = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var proc = new Mock<IProcessManager>(MockBehavior.Strict);

        var expectedSha = "sha-for-head";
        client
            .Setup(c => c.GetShaForRefAsync(
                It.IsAny<string>(),
                It.Is<string>(gr => gr == null)))
            .ReturnsAsync(expectedSha);

        var sut = new LocalGitRepo(default(NativePath), client.Object, proc.Object);

        // Act
        var actual = await sut.GetShaForRefAsync();

        // Assert
        actual.Should().Be(expectedSha);
        client.Verify(c => c.GetShaForRefAsync(It.IsAny<string>(), It.Is<string>(gr => gr == null)), Times.Once);
        client.VerifyNoOtherCalls();
        proc.VerifyNoOtherCalls();
    }

    /// <summary>
    /// Validates that null returned by the underlying ILocalGitClient is propagated by GetShaForRefAsync,
    /// even though the method signature is non-nullable in the source code.
    /// Inputs:
    ...
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task GetShaForRefAsync_UnderlyingReturnsNull_PropagatesNull()
    {
        // Arrange
        var client = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var proc = new Mock<IProcessManager>(MockBehavior.Strict);
    ...
    /// <summary>
    /// Ensures that exceptions thrown by ILocalGitClient.GetShaForRefAsync are propagated unchanged by LocalGitRepo.GetShaForRefAsync.
    /// Inputs:
    ///  - gitRef = "bad/ref".
    /// Expected:
    ///  - The task throws InvalidOperationException.
    ...
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task GetShaForRefAsync_WhenClientThrows_ExceptionIsPropagated()
    {
        // Arrange
        var client = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var proc = new Mock<IProcessManager>(MockBehavior.Strict);
        ...
        // Act
        Func<Task> act = async () => await sut.GetShaForRefAsync(gitRef);
    ...
    private static IEnumerable GetShaForRefAsync_GitRefInputs()
    {
        yield return null;
        yield return "";
        yield return " ";
        yield return "\t\n";
        yield return "HEAD";
        yield return "refs/heads/main";
        yield return "refs/tags/v1.0.0";
        yield return "abc123";
        yield return "äöüß🚀";
        yield return "line1\nline2";
        yield return new string('a', 1024);
        yield return "path/with:invalid*chars?.txt";
    }

    /// <summary>
    /// Verifies that GetCheckedOutBranchAsync delegates to ILocalGitClient.GetCheckedOutBranchAsync
    /// with the repository path and returns exactly the same branch name provided by the client.
    /// Inputs:
    ///  - A variety of branch name strings including empty, whitespace, refs-style, path-like, unicode, and very long values.
    /// Expected:
    ///  - The returned value equals the mocked client return.
    ///  - ILocalGitClient.GetCheckedOutBranchAsync is invoked exactly once.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [TestCaseSource(nameof(BranchNameInputs))]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task GetCheckedOutBranchAsync_VariousBranchNames_DelegatesAndReturnsExpected(string expectedBranch)
    {
        // Arrange
        var clientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);
        ...
        var sut = new LocalGitRepo(default, clientMock.Object, processManagerMock.Object);

        // Act
        var result = await sut.GetCheckedOutBranchAsync();
    ...
    /// <summary>
    /// Validates that if the underlying ILocalGitClient returns null (despite non-nullable signature),
    /// LocalGitRepo.GetCheckedOutBranchAsync propagates the null value as-is.
    /// Inputs:
    ///  - ILocalGitClient configured to return null.
    /// Expected:
    ///  - Result is null.
    ///  - Client method is called exactly once.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task GetCheckedOutBranchAsync_ClientReturnsNull_PropagatesNull()
    {
        // Arrange
        var clientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);
    ...
    private static IEnumerable<string> BranchNameInputs()
    {
        yield return "";
        yield return " ";
        yield return "\t\n";
        yield return "main";
        yield return "feature/xyz";
        yield return "refs/heads/main";
        yield return "bugfix\\windows";
    ...
    /// <summary>
    /// Verifies that PullAsync called without arguments uses the default (non-cancelable) CancellationToken
    /// and forwards the call to ILocalGitClient.PullAsync with the repository path.
    /// Inputs:
    ///  - No parameters (default CancellationToken).
    /// Expected:
    ///  - ILocalGitClient.PullAsync is invoked exactly once with any repo path and a non-cancelable, non-requested token.
    ///  - No exception is thrown.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task PullAsync_NoToken_UsesDefaultCancellationTokenAndCallsClientOnce()
    {
        // Arrange
        var localGitClientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);

        var expectedToken = default(CancellationToken);

        localGitClientMock
            .Setup(c => c.PullAsync(
                It.IsAny<string>(),
                It.Is<CancellationToken>(t => t.Equals(expectedToken) && !t.CanBeCanceled && !t.IsCancellationRequested)))
            .Returns(Task.CompletedTask);
        ...
        // Assert
        localGitClientMock.Verify(c => c.PullAsync(
            It.IsAny<string>(),
            It.Is<CancellationToken>(t => t.Equals(expectedToken) && !t.CanBeCanceled && !t.IsCancellationRequested)), Times.Once);
        localGitClientMock.VerifyNoOtherCalls();
        processManagerMock.VerifyNoOtherCalls();
    }

    /// <summary>
    /// Ensures PullAsync forwards the exact CancellationToken instance (canceled or not)
    /// to ILocalGitClient.PullAsync and calls it exactly once.
    /// Inputs:
    ///  - useSpecificToken: whether to pass a specific token or default.
    ///  - preCanceled: whether the specific token is canceled prior to the call.
    /// Expected:
    ///  - ILocalGitClient.PullAsync is invoked once with any repo path and the exact token.
    ///  - No exception is thrown.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [TestCaseSource(nameof(PullAsync_TokenCases))]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task PullAsync_ForwardToken_CallsClientExactlyOnce(bool useSpecificToken, bool preCanceled)
    {
        // Arrange
        var localGitClientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);
    ...
    /// <summary>
    /// Verifies that exceptions thrown by ILocalGitClient.PullAsync are propagated unchanged by LocalGitRepo.PullAsync.
    /// Inputs:
    ///  - ILocalGitClient.PullAsync configured to throw InvalidOperationException with a specific message.
    /// Expected:
    ///  - LocalGitRepo.PullAsync throws the same InvalidOperationException and message.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task PullAsync_WhenClientThrows_ExceptionPropagates()
    {
        // Arrange
        var localGitClientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);

        var expectedMessage = "pull failed";
        ...
        // Act + Assert
        var ex = Assert.ThrowsAsync<InvalidOperationException>(async () => await sut.PullAsync(CancellationToken.None));
    ...
    /// <summary>
    /// Validates that when ILocalGitClient.PullAsync returns a canceled task for the provided token,
    /// LocalGitRepo.PullAsync throws OperationCanceledException upon awaiting.
    /// Inputs:
    ///  - A CancellationToken canceled before the call; the client returns Task.FromCanceled(token).
    /// Expected:
    ///  - LocalGitRepo.PullAsync throws OperationCanceledException.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task PullAsync_WhenClientTaskIsCanceled_ThrowsOperationCanceled()
    {
        // Arrange
        var localGitClientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);
        ...
        // Act + Assert
        Assert.ThrowsAsync<OperationCanceledException>(async () => await sut.PullAsync(token));
    ...
    private static IEnumerable PullAsync_TokenCases()
    {
        yield return new object[] { false, false }; // default token
        yield return new object[] { true, false };  // specific non-canceled token
        yield return new object[] { true, true };   // specific pre-canceled token
    }

    /// <summary>
    /// Verifies that GetStagedFiles forwards the repository path to ILocalGitClient.GetStagedFiles
    /// and returns exactly the same IReadOnlyCollection<string> instance provided by the client.
    /// Inputs:
    ///  - Various staged file lists including empty, single, duplicates, special/Unicode characters, and long names.
    /// Expected:
    ///  - ILocalGitClient.GetStagedFiles is invoked exactly once with any repo path string.
    ///  - The returned collection reference is the exact same instance returned by the client mock.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [TestCaseSource(nameof(GetStagedFiles_Returns_TestCases))]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task GetStagedFiles_ForwardsPathAndReturnsSameInstance(IReadOnlyCollection<string> clientResult)
    {
        // Arrange
        var clientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);
        ...
        clientMock
            .Setup(c => c.GetStagedFiles(It.IsAny<string>()))
            .ReturnsAsync(clientResult);

        var sut = new LocalGitRepo(default(NativePath), clientMock.Object, processManagerMock.Object);

        // Act
        var result = await sut.GetStagedFiles();

        // Assert
        result.Should().BeSameAs(clientResult);
        clientMock.Verify(c => c.GetStagedFiles(It.IsAny<string>()), Times.Once);
        clientMock.VerifyNoOtherCalls();
        processManagerMock.VerifyNoOtherCalls();
    }

    /// <summary>
    /// Ensures that exceptions thrown by ILocalGitClient.GetStagedFiles are propagated unchanged.
    /// Inputs:
    ///  - Client mock configured to throw InvalidOperationException with a known message.
    /// Expected:
    ///  - LocalGitRepo.GetStagedFiles throws the same InvalidOperationException instance.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public void GetStagedFiles_WhenClientThrows_ExceptionPropagates()
    {
        // Arrange
        var clientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);
        ...
        // Act + Assert
        Assert.ThrowsAsync<InvalidOperationException>(async () => await sut.GetStagedFiles())
            .Should().BeSameAs(expected);
    ...
    /// <summary>
    /// Validates that if the underlying client returns null (despite non-nullable signature),
    /// LocalGitRepo.GetStagedFiles propagates the null result to the caller.
    /// Inputs:
    ///  - Client mock returns Task.FromResult<IReadOnlyCollection&lt;string&gt;>(null).
    /// Expected:
    ///  - The result from LocalGitRepo.GetStagedFiles is null.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task GetStagedFiles_ClientReturnsNull_PropagatesNull()
    {
        // Arrange
        var clientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);
        ...
        clientMock
            .Setup(c => c.GetStagedFiles(It.IsAny<string>()))
            .Returns(Task.FromResult<IReadOnlyCollection<string>>(null));
    ...
    /// <summary>
    /// Verifies that StageAsync forwards the exact IEnumerable instance and the provided CancellationToken
    /// to ILocalGitClient.StageAsync, and completes successfully.
    /// Inputs:
    ///  - pathsToStage: variants including empty, single item, multiple, duplicates, whitespace, special/unicode, very long path.
    ///  - useSpecificToken: whether to pass a specific CancellationToken instance instead of default.
    ///  - preCanceled: whether the specific token is canceled before invocation.
    /// Expected:
    ///  - ILocalGitClient.StageAsync is invoked exactly once with:
    ///      * repoPath: any string derived from LocalGitRepo.Path,
    ///      * the exact same IEnumerable<string> instance provided to StageAsync,
    ///      * the exact CancellationToken instance.
    ///  - No exception is thrown.
    /// </summary>
    [Test]
    [TestCaseSource(nameof(StageAsync_PathsAndToken_Cases))]
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task StageAsync_ForwardsPathsAndToken_InvokesClientExactlyOnce(IEnumerable<string> pathsToStage, bool useSpecificToken, bool preCanceled)
    {
        // Arrange
        var localGitClientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);
        ...
        localGitClientMock
            .Setup(c => c.StageAsync(
                It.IsAny<string>(),
                It.Is<IEnumerable<string>>(e => ReferenceEquals(e, pathsToStage)),
                It.Is<CancellationToken>(t => t.Equals(token))))
            .Returns(Task.CompletedTask);
        ...
        // Act
        await sut.StageAsync(pathsToStage, token);

        // Assert
        localGitClientMock.Verify(c => c.StageAsync(
            It.IsAny<string>(),
            It.Is<IEnumerable<string>>(e => ReferenceEquals(e, pathsToStage)),
            It.Is<CancellationToken>(t => t.Equals(token))), Times.Once);
        localGitClientMock.VerifyNoOtherCalls();
    ...
    /// <summary>
    /// Ensures that exceptions thrown by ILocalGitClient.StageAsync are not swallowed and are propagated by LocalGitRepo.StageAsync.
    /// Inputs:
    ///  - A non-empty paths collection and default CancellationToken.
    /// Expected:
    ///  - The same InvalidOperationException instance thrown by the client is observed by the caller.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public void StageAsync_WhenClientThrows_ExceptionPropagates()
    {
        // Arrange
        var localGitClientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);

        var paths = new[] { "file1.txt", "file2.cs" };
        var token = default(CancellationToken);
        var expected = new InvalidOperationException("boom");
        ...
        localGitClientMock
            .Setup(c => c.StageAsync(
                It.IsAny<string>(),
                It.Is<IEnumerable<string>>(e => ReferenceEquals(e, paths)),
                It.Is<CancellationToken>(t => t.Equals(token))))
            .Throws(expected);
        ...
        // Act + Assert
        var thrown = Assert.ThrowsAsync<InvalidOperationException>(async () => await sut.StageAsync(paths, token));
        thrown.Should().BeSameAs(expected);
    ...
    /// <summary>
    /// Verifies that when ILocalGitClient.StageAsync returns a canceled Task for the provided token,
    /// LocalGitRepo.StageAsync throws OperationCanceledException.
    /// Inputs:
    ///  - A paths collection.
    ///  - A pre-canceled CancellationToken.
    /// Expected:
    ///  - StageAsync throws OperationCanceledException.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task StageAsync_WhenClientTaskIsCanceled_ThrowsOperationCanceledException()
    {
        // Arrange
        var localGitClientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);
        ...
        // Act + Assert
        var thrown = Assert.ThrowsAsync<OperationCanceledException>(async () => await sut.StageAsync(paths, token));
    ...
    private static IEnumerable StageAsync_PathsAndToken_Cases()
    {
        yield return new object[] { Array.Empty<string>(), false, false };
        yield return new object[] { new[] { "file.txt" }, true, false };
        yield return new object[] { new[] { "dup.txt", "dup.txt", " ", "\t\n", "特殊/字符.txt", "path/with:invalid*chars?.txt", new string('a', 1024) }, false, false };
        yield return new object[] { new List<string> { "./relative/../a.cs", "C:\\abs\\b.cs" }, true, true };
        yield return new object[] { new[] { "a", "b", "c" }, true, false };
    }

    /// <summary>
    /// Verifies that UpdateRemoteAsync forwards the repository path, the exact remoteName, and the exact CancellationToken
    /// to ILocalGitClient.UpdateRemoteAsync. Ensures no exception is thrown for various remoteName values, including
    /// empty, whitespace, special characters, and very long strings, and for both default and specific tokens.
    /// Inputs:
    ///  - remoteName: "", "  ", "origin", "path/with/slash", "weird!@#", 256-char 'a' string.
    ///  - useSpecificToken: false/true (whether to pass a specific token instance).
    ///  - preCanceled: false/true (whether the specific token is already canceled).
    /// Expected:
    ///  - ILocalGitClient.UpdateRemoteAsync is invoked exactly once with any repo path, the exact remoteName, and the exact token.
    ///  - The call completes without throwing.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [TestCaseSource(nameof(UpdateRemoteAsync_Delegates_Cases))]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task UpdateRemoteAsync_ForwardsParameters_DelegatesToClient(string remoteName, bool useSpecificToken, bool preCanceled)
    {
        // Arrange
        var localGitClientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);
        ...
        localGitClientMock
            .Setup(c => c.UpdateRemoteAsync(
                It.IsAny<string>(),
                It.Is<string>(rn => rn == remoteName),
                It.Is<CancellationToken>(t => t.Equals(token))))
            .Returns(Task.CompletedTask);
        ...
        // Act
        await sut.UpdateRemoteAsync(remoteName, token);

        // Assert
        localGitClientMock.Verify(c => c.UpdateRemoteAsync(
            It.IsAny<string>(),
            It.Is<string>(rn => rn == remoteName),
            It.Is<CancellationToken>(t => t.Equals(token))), Times.Once);
        localGitClientMock.VerifyNoOtherCalls();
    ...
    private static IEnumerable<object[]> UpdateRemoteAsync_Delegates_Cases()
    {
        yield return new object[] { "origin", false, false };
        yield return new object[] { "", true, false };
        yield return new object[] { "  ", true, true };
        yield return new object[] { "path/with/slash", false, false };
        yield return new object[] { "weird!@#$%^&*()_+-={}[]|\\:;\"'<>,.?/", true, false };
        yield return new object[] { new string('a', 256), false, false };
    }

    /// <summary>
    /// Ensures that exceptions thrown by ILocalGitClient.UpdateRemoteAsync are propagated unchanged
    /// by LocalGitRepo.UpdateRemoteAsync.
    /// Inputs:
    ///  - remoteName: "origin"
    ///  - cancellationToken: default
    /// Expected:
    ///  - InvalidOperationException with message "boom" is thrown and is the same instance as thrown by the client mock.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public void UpdateRemoteAsync_ClientThrows_ExceptionPropagates()
    {
        // Arrange
        var localGitClientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);
        ...
        // Act
        var ex = Assert.ThrowsAsync<InvalidOperationException>(async () => await sut.UpdateRemoteAsync(remoteName, token));
    ...
    /// <summary>
    /// Verifies that when the underlying client returns a canceled task,
    /// LocalGitRepo.UpdateRemoteAsync throws OperationCanceledException and
    /// preserves the CancellationToken in the exception.
    /// Inputs:
    ///  - remoteName: "upstream"
    ///  - cancellationToken: a pre-canceled token.
    /// Expected:
    ///  - OperationCanceledException is thrown.
    ///  - The exception's CancellationToken equals the provided token.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public void UpdateRemoteAsync_WhenClientTaskIsCanceled_ThrowsOperationCanceledWithToken()
    {
        // Arrange
        var localGitClientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);
        ...
        // Act
        var ex = Assert.ThrowsAsync<OperationCanceledException>(async () => await sut.UpdateRemoteAsync(remoteName, token));

        // Assert
        ex.CancellationToken.Should().Be(token);
        localGitClientMock.Verify(c => c.UpdateRemoteAsync(
            It.IsAny<string>(),
            It.Is<string>(rn => rn == remoteName),
            It.Is<CancellationToken>(t => t.Equals(token))), Times.Once);
    ...
    /// <summary>
    /// Ensures that exceptions thrown by the underlying ILocalGitClient.GetRefType
    /// are not swallowed and propagate to the caller.
    /// Inputs:
    ///  - A gitRef string ("refs/heads/main").
    ///  - A default CancellationToken.
    /// Expected:
    ///  - InvalidOperationException is thrown from LocalGitRepo.GetRefType.
    /// </summary>
    [Test]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    [Category("auto-generated")]
    public async Task GetRefType_WhenClientThrows_PropagatesException()
    {
        // Arrange
        var localGitClientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);
        ...
        localGitClientMock
            .Setup(m => m.GetRefType(
                It.IsAny<string>(),
                It.IsAny<string>(),
                It.IsAny<CancellationToken>()))
            .Returns(Task.FromException<GitObjectType>(new InvalidOperationException("boom")));

        var repo = new LocalGitRepo(default, localGitClientMock.Object, processManagerMock.Object);

        // Act
        Func<Task> act = async () => await repo.GetRefType("refs/heads/main", CancellationToken.None);

        // Assert
        await act.Should().ThrowAsync<InvalidOperationException>();
        localGitClientMock.Verify(m => m.GetRefType(
                It.IsAny<string>(),
                It.IsAny<string>(),
                It.IsAny<CancellationToken>()),
            Times.Once);
...
public class LocalGitRepoResolveConflictTests
{
    /// <summary>
    /// Ensures that exceptions thrown by ILocalGitClient.ResolveConflict are not swallowed and are propagated by LocalGitRepo.ResolveConflict.
    /// Inputs:
    ///  - filePath: "conflicted.txt"
    ///  - ours: true
    /// Expected:
    ///  - InvalidOperationException is thrown from LocalGitRepo.ResolveConflict.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public void ResolveConflict_ClientThrows_ExceptionPropagates()
    {
        // Arrange
        var localGitClientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);

        var filePath = "conflicted.txt";
        var ours = true;
        var expected = new InvalidOperationException("boom");

        localGitClientMock
            .Setup(m => m.ResolveConflict(
                It.IsAny<string>(),
        ...
        // Act + Assert
        Assert.ThrowsAsync<InvalidOperationException>(async () => await repo.ResolveConflict(filePath, ours));
        localGitClientMock.Verify(m => m.ResolveConflict(
                It.IsAny<string>(),
                It.Is<string>(p => p == filePath),
                It.Is<bool>(b => b == ours)),
            Times.Once);
    ...
    /// <summary>
    /// Verifies that when the underlying ILocalGitClient.ResolveConflict returns a canceled Task,
    /// LocalGitRepo.ResolveConflict throws OperationCanceledException upon awaiting.
    /// Inputs:
    ///  - filePath: "canceled.txt"
    ///  - ours: false
    /// Expected:
    ///  - OperationCanceledException is thrown.
    /// </summary>
    ...
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public void ResolveConflict_ClientReturnsCanceledTask_ThrowsOperationCanceled()
    {
        // Arrange
        var localGitClientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);

        var filePath = "canceled.txt";
        var ours = false;
        ...
        // Act + Assert
        Assert.ThrowsAsync<OperationCanceledException>(async () => await repo.ResolveConflict(filePath, ours));
        localGitClientMock.Verify(m => m.ResolveConflict(
                It.IsAny<string>(),
                It.Is<string>(p => p == filePath),
                It.Is<bool>(b => b == ours)),
...
public class LocalGitRepoFactoryTests
{
    /// <summary>
    /// Verifies that LocalGitRepoFactory.Create returns an ILocalGitRepo whose Path property
    /// is the exact same NativePath instance provided (reference equality), across varied path strings.
    /// Inputs:
    ///  - repoPath strings including empty, dot, mixed separators, whitespace, unicode, and very long strings.
    /// Expected:
    ///  - Returned instance is non-null and implements ILocalGitRepo.
    ///  - Returned Path reference is the same NativePath instance passed into Create.
    /// </summary>
    [Test]
    [TestCaseSource(nameof(NativePathInputs))]
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public void Create_WithVariedNativePaths_ReturnsRepoWithSamePathInstance(string pathString)
    {
        // Arrange
        var gitClientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);
        var factory = new LocalGitRepoFactory(gitClientMock.Object, processManagerMock.Object);
        var nativePath = new NativePath(pathString);

        // Act
        var repo = factory.Create(nativePath);

        // Assert
        repo.Should().NotBeNull();
        repo.Should().BeAssignableTo<ILocalGitRepo>();
        repo.Path.Should().BeSameAs(nativePath);
    }

    private static IEnumerable<string> NativePathInputs()
    {
        yield return "";
        yield return ".";
        yield return "C:\\repo";
    ...
    /// <summary>
    /// Ensures that the repository produced by LocalGitRepoFactory.Create wires the injected ILocalGitClient
    /// so that client calls are delegated correctly using the same NativePath instance.
    /// Inputs:
    ///  - expectedResult: whether the mocked client reports staged changes (true/false).
    /// Expected:
    ///  - Calling HasStagedChangesAsync on the created repo returns the same boolean.
    ///  - ILocalGitClient.HasStagedChangesAsync is invoked exactly once with the same NativePath instance.
    /// </summary>
    [TestCase(true)]
    [TestCase(false)]
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task Create_ReturnedRepoUsesInjectedClient_ForHasStagedChanges(bool expectedResult)
    {
        // Arrange
        var gitClientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);
        ...
        var nativePath = new NativePath("C:/repo");
        gitClientMock
            .Setup(c => c.HasStagedChangesAsync(It.IsAny<NativePath>()))
            .ReturnsAsync(expectedResult);
        ...
        gitClientMock.Verify(
            c => c.HasStagedChangesAsync(It.Is<NativePath>(p => object.ReferenceEquals(p, nativePath))),
            Times.Once);
    }

    /// <summary>
    /// Verifies that LocalGitRepoFactory.Create constructs a LocalGitRepo with:
    /// - The exact provided NativePath (normalized by NativePath),
    /// - The injected ILocalGitClient (verified by forwarding GetGitCommitAsync),
    /// - And returns an object implementing ILocalGitRepo.
    /// Inputs:
    ///  - repoPathStr: Various path strings including empty, whitespace, relative, absolute-like, special/unicode, and long paths.
    /// Expected:
    ///  - Returned instance is not null and is of type LocalGitRepo.
    ///  - Calling GetGitCommitAsync forwards the normalized path string to ILocalGitClient.GetGitCommitAsync.
    ///  - The returned commit string equals the mock's configured value.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [TestCaseSource(nameof(Create_RepoPath_Variants))]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public async Task Create_WithVariousRepoPaths_ConstructsLocalGitRepoAndWiresDependencies(string repoPathStr)
    {
        // Arrange
        var expectedPath = new NativePath(repoPathStr).Path;
        var expectedCommit = "commit-from-client";

        string observedRepoPath = null;

        var clientMock = new Mock<ILocalGitClient>(MockBehavior.Strict);
        clientMock
            .Setup(m => m.GetGitCommitAsync(It.IsAny<string?>(), It.IsAny<CancellationToken>()))
            .Callback<string?, CancellationToken>((p, _) => observedRepoPath = p)
            .ReturnsAsync(expectedCommit);

        var processManagerMock = new Mock<IProcessManager>(MockBehavior.Strict);

        var factory = new LocalGitRepoFactory(clientMock.Object, processManagerMock.Object);
        var repo = factory.Create(new NativePath(repoPathStr));

        // Act
        var commit = await repo.GetGitCommitAsync(CancellationToken.None);

        // Assert
        repo.Should().NotBeNull();
        repo.Should().BeOfType<LocalGitRepo>();
        observedRepoPath.Should().Be(expectedPath);
        commit.Should().Be(expectedCommit);

        clientMock.Verify(m => m.GetGitCommitAsync(expectedPath, It.IsAny<CancellationToken>()), Times.Once);
        clientMock.VerifyNoOtherCalls();
        processManagerMock.VerifyNoOtherCalls();
    }

    private static IEnumerable Create_RepoPath_Variants()
    {
        yield return "";
        yield return " ";
        yield return ".";
        ...
        yield return "relative/path/with/subdirs";
        yield return "特殊/路径";
