...
public class GitRepoFactoryTests
{
    ...
    private static System.Collections.Generic.IEnumerable<string> TemporaryPathCases()
    {
        ...
        yield return "路径/путь/مسار";
        yield return "name-with-😊-emoji";
        ...
        out Mock<ILoggerFactory> loggerFactory)
    {
        ...
        var factory = new GitRepoFactory(
            remoteTokenProvider.Object,
            azdoTokenProvider.Object,
            telemetryRecorder.Object,
            processManager.Object,
    ...
    ///  - temporaryPath: "C:\temp\darc"
    /// Expected:
    ///  - No exception is thrown and an instance is created successfully.
    /// </summary>
    [Test]
    [Category("auto-generated")]
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    public void Ctor_ValidDependenciesAndPath_DoesNotThrow()
    {
        // Arrange
        var remoteTokenProvider = new Mock<IRemoteTokenProvider>();
        var azdoTokenProvider = new Mock<IAzureDevOpsTokenProvider>();
        var telemetryRecorder = new Mock<ITelemetryRecorder>();
        ...
        // Assert
        // Intentionally no explicit assertions: successful construction (no exception) is the validation.
    }

    /// <summary>
    /// Verifies that Azure DevOps repository URIs are correctly recognized and produce an AzureDevOpsClient.
    /// Inputs:
    ///  - repoUri: dev.azure.com and *.visualstudio.com URLs.
    /// Expected:
    ///  - Returns an instance of AzureDevOpsClient.
    /// </summary>
    [Test]
    ...
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    [Category("auto-generated")]
    public void CreateClient_AzureDevOpsUris_ReturnsAzureDevOpsClient(string repoUri)
    {
        // Arrange
        var factory = CreateFactory(
            out var azdoTokenProvider,
            out var remoteTokenProvider,
            out var telemetryRecorder,
            out var processManager,
            out var fileSystem,
    ...
    /// <summary>
    /// Verifies that local repository identifiers (relative path or file:// URI) produce a LocalLibGit2Client.
    /// Inputs:
    ///  - repoUri: "local-repo", "file:///C:/repo"
    /// Expected:
    ///  - Returns an instance of LocalLibGit2Client.
    /// </summary>
    [Test]
    ...
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    [Category("auto-generated")]
    public void CreateClient_LocalInputs_ReturnsLocalLibGit2Client(string repoUri)
    {
        // Arrange
        var factory = CreateFactory(
            out var azdoTokenProvider,
    ...
    /// <summary>
    /// Ensures that unsupported or invalid repository identifiers cause an ArgumentException.
    /// Inputs (repoUri):
    ///  - null
    ///  - "https://example.com/org/repo" (unknown host)
    ///  - "foo|bar" (invalid path chars)
    ///  - "abc\0def" (control char)
    /// Expected:
    ///  - Throws ArgumentException with parameter name "repoUri" and message containing "Unknown git repository type".
    /// </summary>
    [Test]
    [TestCase(null)]
    [TestCase("https://example.com/org/repo")]
    ...
    [Author("Code Testing Agent v0.3.0-alpha.25425.8+159f94d")]
    [Category("auto-generated")]
    public void CreateClient_UnsupportedOrInvalidRepoUri_ThrowsArgumentException(string repoUri)
    {
        // Arrange
        var factory = CreateFactory(
            out var azdoTokenProvider,
        ...
        act.Should()
           ...
           .Which.ParamName.Should().Be("repoUri");
        act.Should()
           .Throw<ArgumentException>()
           .WithMessage("*Unknown git repository type*");
    }
    ...
    private static GitRepoFactory CreateFactory(
        ...
        string temporaryPath)
    {
        azdoTokenProvider = new Mock<IAzureDevOpsTokenProvider>(MockBehavior.Loose);
        remoteTokenProvider = new Mock<IRemoteTokenProvider>(MockBehavior.Loose);
        telemetryRecorder = new Mock<ITelemetryRecorder>(MockBehavior.Loose);
        processManager = new Mock<IProcessManager>(MockBehavior.Loose);
        fileSystem = new Mock<IFileSystem>(MockBehavior.Loose);

        var loggerMock = new Mock<ILogger>(MockBehavior.Loose);
        loggerFactory = new Mock<ILoggerFactory>(MockBehavior.Loose);
