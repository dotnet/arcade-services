    ...
    public async Task Subscriptions_EndToEnd()
    {
        ...
        await using (AsyncDisposableValue<string> channel1 = await CreateTestChannelAsync(channel1Name))
        {
            await using (AsyncDisposableValue<string> channel2 = await CreateTestChannelAsync(channel2Name))
            {
                ...
                (await GetSubscriptionInfo(subscription3Id.Value)).ShouldContain("Enabled: True", customMessage: $"Expected subscription {subscription3Id} to be enabled");
                ...
                await Should.ThrowAsync<ScenarioTestException>(async () => await GetSubscriptions(channel1Name), "Subscriptions for channel 1 were not deleted.");
                ...
                await Should.ThrowAsync<ScenarioTestException>(async () =>
